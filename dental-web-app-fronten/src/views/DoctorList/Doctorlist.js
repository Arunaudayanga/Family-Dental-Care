...
export default function Doctorlist() {
    ...
    useEffect(() => {
        const loadPosts =async () =>{
            setLoading(true);
            const response =await axios.get("http://localhost:8070/DoctorList/");
    ...
    useEffect(() => {
       function getDoctorLists() {
           axios.get("http://localhost:8070/DoctorList/").then((res) =>{
               setDoctorLists(res.data);
           }).catch((err) =>{
               alert(err.message);
           })
       }
    ...
    function deleteDoctorList(_id) {
        axios.delete("http://localhost:8070/DoctorList/delete/"+_id).then((res) =>{
        ...
        setDoctorLists(DoctorLists.filter((DoctorLists) => DoctorLists._id !== _id))
    }
    ...
    function createPdf(pdfBody){
        ...
        doc.autoTable({
            didDrawPage: function (data) {
                ...
                // Header
                doc.setFontSize(14);
                var fileTitle = "DoctorList Report";
        ...
        doc.save('DoctorList.pdf'); //this downloads a copy of the pdf in your local instance.
    };
    ...
    return (
        <><div className ="DoctorListBody">
           
            
        </div><div className ="container">
        <h1>All DoctorLists</h1>
        <input className="searchDoctorList"
            style={{ width:"15%" ,height:"30px"}}
            type="text"
            placeholder="Search Doctor"
            onChange={(e) => setSearchName(e.target.value)}/>

        <table id="my-table" class="table table-bordered DoctorListTable table-light">
                    ...
                    posts
                        .filter((value) => {
                            if (searchName === "") {
                                ...
                                value.doctor_name.includes(searchName.toUpperCase())
                            ){
                                return value;
                            }
